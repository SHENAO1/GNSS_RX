...
这说明 `mkdir -p "<img src="https://latex.codecogs.com/gif.latex?CAPTURE_DIR&quot;`%20已成功，后续%20TX%20dry-run%20导出%20truth%20与%20RX%20正式采集都可以直接沿用这三个变量。新规则里%20`RUN_TS`%20负责秒级文件名时间戳；更精确的开始时间会写入%20`.json`%20的%20`capture_started_at_iso`。推荐先在%20TX%20侧做一次%20dry-run，把%20sidecar%20truth%20直接导出到本轮采集目录：```bashcd%20~/projects/gnss_txsource%20.venv/bin/activatePYTHONPATH=src%20python3%20scripts/run_tx.py%20\%20%20%20%20--config%20configs/tx_b210_cable_loopback.yaml%20\%20%20%20%20--tx-gain%2050%20\%20%20%20%20--amplitude%201.0%20\%20%20%20%20--dry-run%20\%20%20%20%20--export-truth-json%20&quot;"/>TRUTH_PATH"
deactivate
```
...
- `printf 'TRUTH_PATH=<%s>\n' "<img src="https://latex.codecogs.com/gif.latex?TRUTH_PATH&quot;`%20能打印出完整绝对路径，且以%20`_tx_truth.json`%20结尾-%20重新运行本节%20dry-run%20后，日志里出现%20`[INFO]%20Exported%20TX%20truth%20JSON:%20...`-%20执行%20`ls%20-l%20&quot;"/>TRUTH_PATH"` 能看到该 JSON 文件已经落盘
...
- 移动硬盘挂载点：`/media/<img src="https://latex.codecogs.com/gif.latex?USER/&lt;drive_name&gt;/GNSS_RX_matlab`-%20Ubuntu%20本机临时目录：`"/>HOME/GNSS_RX_matlab`
  
若这里直接运行 `./scripts/sync_matlab.sh ...` 出现“权限不够”，通常只是脚本暂时没有执行位；优先改用 `bash ./scripts/sync_matlab.sh ...` 即可继续。
  
若目标写成 `/mnt/hgfs/...` 却报 `mkdir: 无法创建目录 "/mnt/hgfs": 权限不够`，优先按“该机没有 VMware 共享目录挂载”处理。此时不要继续硬用 `/mnt/hgfs/...`，而应改成这台 Ubuntu 当前真实存在的可写目录；如果本轮走移动硬盘转运，推荐直接改成 `/media/<img src="https://latex.codecogs.com/gif.latex?USER/&lt;drive_name&gt;/GNSS_RX_matlab`。同步内容包括：-%20`functions/`-%20`scripts/`-%20`README.md`-%20`architecture.drawio`-%20`gnss_rx_user_paths.m.example`-%20根目录快捷入口%20`ber.m`注意：-%20`GNSS_RX/matlab/`%20才是源码真相源-%20`GNSS_RX_matlab/`%20只是主力机部署镜像，不应手工改代码-%20`sync_matlab.sh`%20只同步%20MATLAB%20代码与受管入口文件-%20采集目录中的%20sidecar%20truth%20属于数据，不属于%20MATLAB%20代码镜像，不会随这个脚本一起复制###%2012.2%20为什么要先同步因为正式%20BER%20现在以：-%20`ber`-%20`run_ber_loopback.m`为主入口，若主力机%20MATLAB%20还在用旧部署副本，就会出现：-%20路径指向旧函数-%20`ber`%20不存在-%20`run_ber_loopback.m`%20与仓库当前逻辑不一致###%2012.3%20在%20MATLAB%20中确认已加载新版本进入主力机%20MATLAB%20后，先执行：```matlabwhich%20ber%20-allwhich%20run_ber_loopback%20-allwhich%20run_prn_acquisition%20-allwhich%20recover_nav_bits%20-allwhich%20gnss_rx_resolve_accel_options%20-all```要求这些路径都指向本轮刚同步的工作区。---##%20十三、先做离线固定样本%20BER%20回归这一步不需要真实%20TX/RX%20同时开机。目标不是重采，而是先验证：-%20MATLAB%20环境通-%20`ber`%20入口通-%20`run_ber_loopback.m`%20主链通-%20truth%20自动匹配口径通-%20`tracked_truth`%20判决通###%2013.1%20固定回归样本当前历史基线样本：```textC:\VMwareVirtualMachines\GongXiangDocument\GNSS_RX_Data\2026\2026_03_28\20260328_142122_rawiq_sc16_zeroif_prn1_spread_sr4092000_cf100000000_dur30p0s```本轮回归时建议显式指定%20`CAPTURE_PATH`，不要依赖“自动选最新文件”。###%2013.2%20Windows%20主力机正式%20BER%20示例```matlabcd(&#39;C:\VMwareVirtualMachines\GongXiangDocument\GNSS_RX_matlab&#39;)clear%20functionsrehashCAPTURE_PATH%20=%20[&#39;C:\VMwareVirtualMachines\GongXiangDocument\GNSS_RX_Data\2026\&#39;%20...%20%20%20%20&#39;2026_03_28\20260328_142122_rawiq_sc16_zeroif_prn1_spread_sr4092000_&#39;%20...%20%20%20%20&#39;cf100000000_dur30p0s\20260328_142122_rawiq_sc16_zeroif_prn1_spread_&#39;%20...%20%20%20%20&#39;sr4092000_cf100000000_dur30p0s&#39;];BER_MODE%20=%20&#39;tracked_truth&#39;;ber```###%2013.3%20Linux%20主力机正式%20BER%20示例```matlabcd(&#39;/home/shenao/projects/GNSS_RX/matlab&#39;)clear%20functionsrehashCAPTURE_PATH%20=%20[&#39;/mnt/hgfs/GongXiangDocument/GNSS_RX_Data/2026/&#39;%20...%20%20%20%20&#39;2026_03_28/20260328_142122_rawiq_sc16_zeroif_prn1_spread_sr4092000_&#39;%20...%20%20%20%20&#39;cf100000000_dur30p0s/20260328_142122_rawiq_sc16_zeroif_prn1_spread_&#39;%20...%20%20%20%20&#39;sr4092000_cf100000000_dur30p0s&#39;];BER_MODE%20=%20&#39;tracked_truth&#39;;run(&#39;scripts/run_ber_loopback.m&#39;)```###%2013.4%20快速体检入口只作为补充若你只想先快速确认样本能否加载、捕获和%20survey，可运行：```matlabresult%20=%20run_capture_analysis(CAPTURE_PATH);```但必须明确：-%20`run_capture_analysis()`%20不是正式%20BER%20入口-%20它不替代%20`ber`-%20它适合先看“采集文件有没有信号、PRN%20捕获是否成功、总览图是否异常”###%2013.5%20当前最低通过标准-%20日志显示%20`TX%20truth：JSON%20模式`-%20日志进入%20`===%20Step%204:%20tracked%20BER%20主链%20===`-%20`BER_MODE=&#39;tracked_truth&#39;`-%20`tracked%20BER`%20有效输出若固定%2030%20s%20样本都无法收敛，应先停在这里排软件链，而不是马上重采。---##%20十四、联机%2030%20s%20复验这是第一轮真实%20TX/RX%20同时参与的实验。###%2014.1%20启动规则-%20TX%20先启动-%20RX%20后启动-%20TX%20发射时长必须长于%20RX%20采集时长-%2030%20s%20采集时，TX%20推荐给%20`60%20s`###%2014.2%20TX%20端命令```bashcd%20~/projects/gnss_txsource%20.venv/bin/activatePYTHONPATH=src%20python3%20scripts/run_tx.py%20\%20%20%20%20--config%20configs/tx_b210_cable_loopback.yaml%20\%20%20%20%20--tx-gain%2050%20\%20%20%20%20--amplitude%201.0%20\%20%20%20%20--duration%2060```###%2014.3%20RX%20端命令TX%20启动稳定后约%205%20秒，再开%20RX：```bashcd%20~/projects/GNSS_RXsource%20.venv/bin/activateRUN_TS="/>(date +%Y%m%d_%H%M%S)
CAPTURE_NAME=<img src="https://latex.codecogs.com/gif.latex?{RUN_TS}_ber30s_prn1_spread_sr4p092e6_cf100e6_d30sCAPTURE_DIR=/home/"/>USER/GNSS_RX_Data_local/2026/2026_03_31/<img src="https://latex.codecogs.com/gif.latex?CAPTURE_NAMELOCAL_STEM="/>CAPTURE_DIR/<img src="https://latex.codecogs.com/gif.latex?CAPTURE_NAMETRUTH_PATH="/>CAPTURE_DIR/<img src="https://latex.codecogs.com/gif.latex?{CAPTURE_NAME}_tx_truth.jsonmkdir%20-p%20&quot;"/>CAPTURE_DIR"
printf 'LOCAL_STEM=<%s>\n' "<img src="https://latex.codecogs.com/gif.latex?LOCAL_STEM&quot;printf%20&#39;TRUTH_PATH=&lt;%s&gt;\n&#39;%20&quot;"/>TRUTH_PATH"
  
PYTHONPATH=src python3 scripts/record_rx.py \
    --config configs/rx_baremetal.yaml \
    --duration 30 \
    --capture-mode single \
    --output-stem "<img src="https://latex.codecogs.com/gif.latex?LOCAL_STEM&quot;```###%2014.4%20文件量级`30%20s%20@%204.092%20Msps`%20的%20`.sc16`%20大约为：-%20约%20`0.49%20GB`%20十进制-%20约%20`0.46%20GiB`%20二进制###%2014.5%20MATLAB%20复验口径这轮分析时，`CAPTURE_PATH`%20应切到“刚刚新采集的%2030%20s%20样本”，不要继续分析旧基线文件。###%2014.6%20完成标志-%20新的%2030%20s%20采集成功生成-%20TX%20无%20underflow-%20RX%20无%20overflow-%20MATLAB%20上%20`tracked_truth`%20BER%20低误码-%20连续%203%20份新%2030%20s%20样本都稳定---##%20十五、联机%20100%20s%20过渡验证这一步是%2030%20s%20到%20250%20s%20的过渡桥，不建议跳过。目的：-%20验证“本地落盘%20+%20采后复制”流程-%20验证系统负载拉长后仍无%20overflow%20/%20underflow-%20避免一上来就把%20250%20s%20或%201%20h%20的问题混在一起###%2015.1%20TX%20端命令```bashcd%20~/projects/gnss_txsource%20.venv/bin/activatePYTHONPATH=src%20python3%20scripts/run_tx.py%20\%20%20%20%20--config%20configs/tx_b210_cable_loopback.yaml%20\%20%20%20%20--tx-gain%2050%20\%20%20%20%20--amplitude%201.0%20\%20%20%20%20--duration%20120```###%2015.2%20RX%20端命令推荐显式写本地输出%20stem：```bashcd%20~/projects/GNSS_RXsource%20.venv/bin/activateRUN_TS="/>(date +%Y%m%d_%H%M%S)
CAPTURE_NAME=<img src="https://latex.codecogs.com/gif.latex?{RUN_TS}_ber100s_prn1_spread_sr4p092e6_cf100e6_d100sCAPTURE_DIR=/home/"/>USER/GNSS_RX_Data_local/2026/2026_03_31/<img src="https://latex.codecogs.com/gif.latex?CAPTURE_NAMELOCAL_STEM="/>CAPTURE_DIR/<img src="https://latex.codecogs.com/gif.latex?CAPTURE_NAMETRUTH_PATH="/>CAPTURE_DIR/<img src="https://latex.codecogs.com/gif.latex?{CAPTURE_NAME}_tx_truth.jsonmkdir%20-p%20&quot;"/>CAPTURE_DIR"
printf 'LOCAL_STEM=<%s>\n' "<img src="https://latex.codecogs.com/gif.latex?LOCAL_STEM&quot;printf%20&#39;TRUTH_PATH=&lt;%s&gt;\n&#39;%20&quot;"/>TRUTH_PATH"
...
PYTHONPATH=src python3 scripts/record_rx.py \
    ...
    --duration 100 \
    --capture-mode single \
    --output-stem "<img src="https://latex.codecogs.com/gif.latex?LOCAL_STEM&quot;```###%2015.3%20文件量级`100%20s%20@%204.092%20Msps`%20的%20`.sc16`%20大约为：-%20约%20`1.64%20GB`%20十进制-%20约%20`1.52%20GiB`%20二进制###%2015.4%20完成标志-%20100%20s%20成功写入本地磁盘-%20TX%20无%20underflow-%20RX%20无%20overflow-%20后续复制和%20MATLAB%20分析都正常---##%20十六、联机%20250%20s%20正式%20BER%20验收`250%20s`%20是当前正式%20BER%20验收区间。###%2016.1%20为什么不建议直接写共享目录历史经验已经表明，长时采集若直接写：```text/mnt/hgfs/...```更容易把共享目录写盘抖动和主机负载混进来，增加%20overflow%20风险。因此%20250%20s%20统一推荐流程是：```text先写裸机本地磁盘→%20采集结束→%20再复制到共享目录或移动硬盘→%20再给主力机%20MATLAB%20分析```若笔记本只有两个%20USB%203.x%20口，并且这两个口都被两台%20B210%20占用，则采集阶段不要强行插入移动硬盘。默认先写入：```text/home/"/>USER/GNSS_RX_Data_local/...
```
...
本轮 `ls /media/<img src="https://latex.codecogs.com/gif.latex?USER`%20已实测为：```textSeagate%20Basic```因此当前可直接写成：```bashDRIVE_ROOT=&quot;/media/"/>USER/Seagate Basic"
TODAY_YEAR=<img src="https://latex.codecogs.com/gif.latex?(date%20+%Y)TODAY_DIR="/>(date +%Y_%m_%d)
DEST_PARENT="<img src="https://latex.codecogs.com/gif.latex?DRIVE_ROOT/GNSS_RX_Data_local/"/>TODAY_YEAR/<img src="https://latex.codecogs.com/gif.latex?TODAY_DIR&quot;DEST_DIR=&quot;"/>DEST_PARENT/<img src="https://latex.codecogs.com/gif.latex?CAPTURE_NAME&quot;mkdir%20-p%20&quot;"/>DEST_PARENT"
cp -av "<img src="https://latex.codecogs.com/gif.latex?CAPTURE_DIR&quot;%20&quot;"/>DEST_PARENT"/
sync
ls -lh "<img src="https://latex.codecogs.com/gif.latex?DEST_DIR&quot;```复制完成后，`"/>DEST_DIR` 下应继续同时包含：
...
- 若后面确实遇到权限问题，再单独执行 `sudo chown -R <img src="https://latex.codecogs.com/gif.latex?USER:"/>USER "<img src="https://latex.codecogs.com/gif.latex?CAPTURE_DIR&quot;`%20修正属主###%2016.6%20文件量级`250%20s%20@%204.092%20Msps`%20的%20`.sc16`%20大约为：-%20约%20`4.09%20GB`%20十进制-%20约%20`3.81%20GiB`%20二进制###%2016.6.1%20移动硬盘回到%20Windows%20后的%20MATLAB%20验证命令当移动硬盘重新插回%20Windows%20主力%20MATLAB%20分析机后，推荐先不要直接跑%20`ber`，而是先在%20MATLAB%20命令行里完成下面这组验证。先确认%20Windows%20端%20MATLAB%20代码目录：```matlabCODE_ROOT%20=%20&#39;E:\MATLAB_code_Gongwei_Local\GNSS_RX_matlab&#39;;cd(CODE_ROOT)addpath(pwd)addpath(fullfile(pwd,%20&#39;functions&#39;))addpath(fullfile(pwd,%20&#39;scripts&#39;))rehashwhich%20ber%20-allwhich%20run_ber_loopback%20-allwhich%20run_prn_acquisition%20-allwhich%20recover_nav_bits%20-allwhich%20gnss_rx_resolve_accel_options%20-all```本轮已实测通过，下面这组输出就表示“代码侧验证通过，可以继续往下执行”：```textE:\MATLAB_code_Gongwei_Local\GNSS_RX_matlab\ber.mE:\MATLAB_code_Gongwei_Local\GNSS_RX_matlab\scripts\run_ber_loopback.mE:\MATLAB_code_Gongwei_Local\GNSS_RX_matlab\functions\run_prn_acquisition.mE:\MATLAB_code_Gongwei_Local\GNSS_RX_matlab\functions\recover_nav_bits.mE:\MATLAB_code_Gongwei_Local\GNSS_RX_matlab\functions\gnss_rx_resolve_accel_options.m```只要%20`which%20...%20-all`%20全部指向%20`E:\MATLAB_code_Gongwei_Local\GNSS_RX_matlab\...`，就说明：-%20MATLAB%20已经加载到本轮正确代码副本-%20不再受旧目录或网络盘%20`Z:`%20干扰-%20可以继续执行下面的“移动硬盘盘符确认”和“采集三件套验证”若不确定移动硬盘当前在%20Windows%20上的盘符，可以先在%20MATLAB%20中执行：```matlabsystem(&#39;powershell%20-NoProfile%20-Command%20&quot;Get-Volume%20|%20Select%20DriveLetter,%20FileSystemLabel%20|%20Format-Table%20-AutoSize&quot;&#39;)```假设移动硬盘当前盘符为%20`F:`，则继续执行：```matlabCAPTURE_DIR%20=%20[&#39;F:\GNSS_RX_Data_local\2026\2026_03_31\&#39;%20...%20%20%20%20&#39;20260331_190530_ber250s_prn1_spread_sr4p092e6_cf100e6_d250s&#39;];CAPTURE_STEM%20=%20fullfile(CAPTURE_DIR,%20...%20%20%20%20&#39;20260331_190530_ber250s_prn1_spread_sr4p092e6_cf100e6_d250s&#39;);dir(CAPTURE_DIR)exist([CAPTURE_STEM%20&#39;.sc16&#39;],%20&#39;file&#39;)exist([CAPTURE_STEM%20&#39;.json&#39;],%20&#39;file&#39;)exist([CAPTURE_STEM%20&#39;_tx_truth.json&#39;],%20&#39;file&#39;)truth%20=%20load_tx_truth_json([CAPTURE_STEM%20&#39;_tx_truth.json&#39;]);disp(truth.prn_id)disp(truth.sample_rate)```本轮已实测通过，出现下面这类结果就表示“采集三件套%20+%20sidecar%20truth%20验证通过”：```textans%20=%20%20%20%20%202ans%20=%20%20%20%20%202ans%20=%20%20%20%20%202%20%20%20%20%201%20%20%20%20%204092000```其中含义为：-%20前面%203%20个%20`ans%20=%202`%20表示：%20%20-%20`[CAPTURE_STEM%20&#39;.sc16&#39;]`%20存在%20%20-%20`[CAPTURE_STEM%20&#39;.json&#39;]`%20存在%20%20-%20`[CAPTURE_STEM%20&#39;_tx_truth.json&#39;]`%20存在-%20`disp(truth.prn_id)`%20输出%20`1`，说明读取到的%20truth%20对应%20`PRN%201`-%20`disp(truth.sample_rate)`%20输出%20`4092000`，说明%20truth%20中的采样率与本轮基线一致只要你看到的是这一类结果，就说明：-%20Windows%20已能正常读取移动硬盘上的本轮采集目录-%20sidecar%20truth%20与本轮文件名绑定正确-%20可以继续进入下面的正式%20`ber`%20步骤这一步的目标不是先出%20BER%20数值，而是先确认：-%20MATLAB%20已从%20`E:\MATLAB_code_Gongwei_Local\GNSS_RX_matlab`%20加载到正确代码-%20移动硬盘上的%20`250%20s`%20目录确实能被%20Windows%20正常读到-%20`.sc16`、`.json`、`_tx_truth.json`%20三件套齐全-%20`load_tx_truth_json(...)`%20能正常解析%20sidecar%20truth以上都通过后，再进入后面的正式%20`ber`%20步骤。###%2016.6.2%20验证通过后的正式%20`ber`%20命令当%20`which%20...%20-all`、`exist(...)`、`load_tx_truth_json(...)`%20都通过后，默认按%20**GPU%20加速优先**%20的版本执行正式%20BER：推荐先单独完成一次“GPU%20启动检查”，再运行%20`ber`：```matlabparallel.gpu.enableCUDAForwardCompatibility(true);gpuDeviceCountg%20=%20gpuDevice;disp(g.Name)disp(g.ComputeCapability)```只要这组命令能正常返回设备对象，就说明本轮%20MATLAB%20已经完成%20GPU%20绑定，可以继续把%20`ACCEL_OPTIONS`%20设为%20GPU。本轮已实测通过，实际输出为：```textans%20=%20%20%20%20%201警告:%20将重新编译%20GPU%20库，因为您的设备比库更新。编译可能需要几分钟时间。NVIDIA%20GeForce%20RTX%20506012.0```这组结果的含义是：-%20`gpuDeviceCount%20=%201`：MATLAB%20已检测到%201%20块可见%20GPU-%20`gpuDevice`%20能成功返回设备对象：说明启用%20`parallel.gpu.enableCUDAForwardCompatibility(true)`%20后，GPU%20已可被当前%20MATLAB%20会话实际使用-%20`NVIDIA%20GeForce%20RTX%205060`：当前绑定到的%20GPU%20设备名称-%20`12.0`：该卡的%20`ComputeCapability`-%20“将重新编译%20GPU%20库”警告：属于%20forward%20compatibility%20模式下的预期现象，首次绑定新架构%20GPU%20时可能需要额外编译时间；只要后续没有报错中断，就不视为失败因此，本轮这组输出应判定为：```textGPU%20启动检查通过，可以继续按%20GPU%20版本运行%20ber``````matlabCODE_ROOT%20=%20&#39;E:\MATLAB_code_Gongwei_Local\GNSS_RX_matlab&#39;;cd(CODE_ROOT)addpath(pwd)addpath(fullfile(pwd,%20&#39;functions&#39;))addpath(fullfile(pwd,%20&#39;scripts&#39;))clear%20functionsrehashparallel.gpu.enableCUDAForwardCompatibility(true);ACCEL_OPTIONS%20=%20struct(%20...%20%20%20%20&#39;backend&#39;,%20&#39;gpu&#39;,%20...%20%20%20%20&#39;precision&#39;,%20&#39;single&#39;,%20...%20%20%20%20&#39;batch_ms&#39;,%202000);DRIVE%20=%20&#39;F:&#39;;%20%20%%20按当前移动硬盘实际盘符替换CAPTURE_STEM%20=%20[DRIVE%20&#39;\GNSS_RX_Data_local\2026\2026_03_31\&#39;%20...%20%20%20%20&#39;20260331_190530_ber250s_prn1_spread_sr4p092e6_cf100e6_d250s\&#39;%20...%20%20%20%20&#39;20260331_190530_ber250s_prn1_spread_sr4p092e6_cf100e6_d250s&#39;];CAPTURE_PATH%20=%20CAPTURE_STEM;BER_MODE%20=%20&#39;tracked_truth&#39;;ber```本轮%20GPU%20相关背景如下：```matlabgpuDevice```本轮实测中，`gpuDeviceCount`%20返回%20`1`，说明%20MATLAB%20能看到%20GPU；但%20`gpuDevice`%20报错提示该卡的%20`compute%20capability%2012.0`%20高于当前%20MATLAB%20内置%20CUDA%20库原生支持范围，因此需要先启用：```matlabparallel.gpu.enableCUDAForwardCompatibility(true)```也就是说，上面正式%20`ber`%20命令里的这两行：```matlabparallel.gpu.enableCUDAForwardCompatibility(true);ACCEL_OPTIONS%20=%20struct(&#39;backend&#39;,%20&#39;gpu&#39;,%20&#39;precision&#39;,%20&#39;single&#39;,%20&#39;batch_ms&#39;,%202000);```就是本轮默认推荐的%20GPU%20版本。结合当前%20MATLAB%20代码实现，可把”GPU%20真正参与了哪些环节”理解为：-%20`run_prn_acquisition`：会在%20GPU%20路径下调用%20`compute_search_map_gpu(...)`，用%20`gpuArray%20+%20FFT/IFFT`%20完成捕获搜索-%20`recover_nav_bits`：会在%20`gpu_enabled=true`%20时，把批量相关求和放到%20GPU%20上执行-%20tracking%20主链（A%20类%20GPU，`gpu_enabled=true`%20时自动激活）：%20%20-%20`estimate_initial_bit_alignment`：20%20种%20bit%20边界%20×%20pattern_len%20×%20极性%20的打分矩阵，GPU%20批量计算%20%20-%20`check_bit_timing_stability`：每个窗口内%2020%20个偏移量的%20bit%20积分，GPU%20向量化%20%20-%20`integrate_bits_from_prompt`：20ms%20积分%20reshape+sum，GPU%20执行-%20tracking%20DLL%20主循环（B%20类%20GPU，需额外设置%20`dll_gpu_enabled=true`）：%20%20-%20`track_code_phase_ms`：批处理段矩阵%20GPU%20矩阵乘，然后%20CPU%20串行更新%20cursor%20%20-%20默认**关闭**，需在%20BER%20验证通过后才推荐开启（见第二十五节）因此本轮判断”GPU%20已生效”的标准，不是要求所有步骤都显示%20GPU，而是看：-%20`gpuDevice`%20能否正常返回设备对象-%20`ACCEL_OPTIONS.backend=&#39;gpu&#39;`%20后，日志中的%20`resolved=gpu`-%20日志中能打印出%20`GPU%20设备：[index]%20name`-%20Step%204%20日志显示%20`Step%204%20实际后端：gpu`代码更新后（tracking%20主链%20A%20类%20GPU%20已激活），`backend=gpu`%20时%20Step%204%20的预期日志格式如下：```text加速配置：requested=gpu,%20resolved=gpu,%20precision=single,%20batch_ms=2000,%20parfor=0GPU%20设备：[1]%20NVIDIA%20GeForce%20RTX%205060===%20Step%202:%20GPS%20L1%20C/A%20捕获%20===Step%202%20后端：gpu（precision=single）捕获成功！Doppler%20=%200.0%20Hz，码相位%20=%202954%20samples，次峰比%20=%20105.54TX%20truth：JSON%20模式（capture%20sidecar%20truth）===%20Step%203:%20open-loop%20truth%20基线%20===Step%203%20后端：gpu（precision=single,%20batch_ms=2000）===%20Step%204:%20tracked%20BER%20主链%20===Step%204%20后端：gputracked%20BER：1.20e-03，匹配率：100.0%，bit%20偏移：15%20ms，pattern%20偏移：7%20bitStep%204%20用时：XX.XX%20sStep%204%20实际后端：gpu========================================%20%20BER：%20%20%20%20%20%20%20%20%201.20e-03%20%20总发送比特数：12499%20%20误码个数：%20%20%20%2015%20%20truth%20匹配率：100.0%%20%20加速后端：%20%20%20%20gpu========================================```关键判断：-%20GPU%20配置已生效：日志显示%20`requested=gpu,%20resolved=gpu`-%20GPU%20已参与%20Step%202%20/%20Step%203：捕获与%20open-loop%20相关计算走%20GPU%20路径-%20Step%204%20tracking%20三个子函数（bit%20对齐打分%20/%20bit%20稳定性检查%20/%20bit%20积分）现在走%20GPU%20路径-%20`Step%204%20实际后端：gpu`%20是本轮新增的日志行，验证%20tracking%20链实际使用了%20GPU-%20`BER%20=%201.20e-03`、`truth%20匹配率%20=%20100.0%`%20应与之前%20CPU%20基线完全一致（Phase%20A%20改动不引入数值近似）-%20sidecar%20truth%20已正确命中：日志显示%20`TX%20truth：JSON%20模式（capture%20sidecar%20truth）`&gt;%20**注意**：若使用%20`backend=cpu`，Step%204%20日志将显示%20`Step%204%20后端：cpu`%20和%20`Step%204%20实际后端：cpu`，这是正常的%20CPU%20路径。本轮末尾还出现过：```text警告:%20将图例条目限制为%2050%20个。```这属于绘图阶段的%20legend%20数量提示，不影响%20BER%20数值本身，也不影响本轮结果判定。若你不想承担%20forward%20compatibility%20的额外不确定性，则继续保持%20CPU%20也完全可行；当前代码默认就是：```matlabACCEL_OPTIONS%20=%20struct();```也就是%20`requested=cpu,%20resolved=cpu,%20precision=double,%20batch_ms=2000,%20parfor=0`。额外说明：-%20从本轮起，若你把%20`ACCEL_OPTIONS.backend`%20设为%20`&#39;auto&#39;`，但%20GPU%20初始化失败，代码会自动回退到%20CPU%20并打印回退原因-%20若你显式设为%20`&#39;gpu&#39;`，则仍保持严格模式，GPU%20初始化失败时直接报错这一步的预期现象是：-%20`ber`%20能正常启动，不报“未找到函数”或“找不到采集文件”-%20日志中优先使用%20capture%20sidecar%20truth-%20BER%20计算开始进入%20acquisition%20/%20tracking%20/%20bit%20recovery%20流程若你希望先做一次更稳妥的显式检查，也可以在%20`ber`%20前先执行：```matlabdisp(CAPTURE_PATH)exist([CAPTURE_PATH%20&#39;.json&#39;],%20&#39;file&#39;)exist([CAPTURE_PATH%20&#39;.sc16&#39;],%20&#39;file&#39;)exist([CAPTURE_PATH%20&#39;_tx_truth.json&#39;],%20&#39;file&#39;)```###%2016.7%20正式验收目标-%20恢复总比特数%20`&gt;=%201e4`-%20总%20BER%20保持低误码-%20无长时间失锁区间-%20TX%20无%20underflow-%20RX%20无%20overflow---##%20十七、联机长时窗口验证（30%20/%2045%20/%2060%20min）###%2017.1%20统一口径从%20`30%20min`%20开始，长时窗口统一固定为：-%20RX%20使用%20`chunked`-%20`--chunk-duration%2030`-%20TX%20保持当前稳定基线-%20TX%20发射覆盖时长必须长于%20RX%20总采集时长-%20MATLAB%20默认先抽查前%20/%20中%20/%20后几个%20chunk，不直接把全部%20chunk%20拼成一次正式%20BER%20结论###%2017.2%2030%20/%2045%20/%2060%20min%20文件大小与%20chunk%20数在当前%20`sc16%20@%204.092%20Msps`%20口径下：|%20时长%20|%20RX%20总时长%20|%20`.sc16`%20总量（十进制）%20|%20`.sc16`%20总量（二进制）%20|%20`30%20s`%20chunk%20数%20|%20单段%20`.sc16`%20量级%20||------|-----------|------------------------|------------------------|-----------------|-------------------||%20`30%20min`%20|%20`1800%20s`%20|%20`29.46%20GB`%20|%20`27.44%20GiB`%20|%20`60`%20|%20约%20`0.49%20GB`%20||%20`45%20min`%20|%20`2700%20s`%20|%20`44.19%20GB`%20|%20`41.16%20GiB`%20|%20`90`%20|%20约%20`0.49%20GB`%20||%20`60%20min`%20|%20`3600%20s`%20|%20`58.92%20GB`%20|%20`54.88%20GiB`%20|%20`120`%20|%20约%20`0.49%20GB`%20|###%2017.3%20当前机器空间判断你当前贴出的%20`df%20-h`%20显示%20`/`%20分区可用空间约为%20`44G`。对应到上表可以直接得到：-%20`30%20min`：当前空间大体可承受-%20`45%20min`：已经非常贴边，不建议当正式目标-%20`60%20min`：当前空间明确不足因此：-%20当前机器最稳妥的长时窗口是先做%20`30%20min`-%20若要做%20`45%20min`，应先进一步清理空间-%20若要做%20`60%20min`，必须先释放空间或把输出目录切到%20Ubuntu%20侧更大的本地磁盘###%2017.4%20推荐执行顺序长时验证默认按下面顺序推进：```text先做%2030%20min→%20稳定后做%2045%20min→%20最后再做%2060%20min```每一轮都固定做五件事：1.%20执行%20TX%20命令2.%20视本轮目标选择%20`chunked`%20或%20`single`3.%20在%20Ubuntu%20侧补导%20truth%20文件4.%20如需离线复盘，复制到移动硬盘%20`Seagate%20Basic`5.%20回传到%20Windows%20后，再跑%20MATLAB%20快速体检或正式%20BER%20命令长时窗口里两种%20RX%20路径的建议口径如下：-%20`chunked`：默认推荐，适合正式长稳验证、抽查前%20/%20中%20/%20后%20chunk、便于搬运-%20`single`：可选验证项，适合确认“一次性连续落盘”是否稳定，但不建议作为默认正式路径MATLAB%20端本章统一给出%203%20类命令：-%20快速体检：`run_capture_analysis(...)`-%20正式%20BER%20主链：`ber`-%20显式脚本入口：`run(&#39;scripts/run_ber_loopback.m&#39;)`默认优先级：-%20快速看文件能否加载：先跑%20`run_capture_analysis(...)`-%20要出正式%20BER：优先跑%20`ber`-%20若需要显式复现实验变量：再跑%20`run(&#39;scripts/run_ber_loopback.m&#39;)`---###%2017.5%20联机%2030%20min####%2017.5.1%20变量块先统一定义%20`chunked`%20和%20`single`%20两套变量：```bashRUN_TS="/>(date +%Y%m%d_%H%M%S)
TODAY_YEAR=<img src="https://latex.codecogs.com/gif.latex?(date%20+%Y)TODAY_DIR="/>(date +%Y_%m_%d)
  
CAPTURE_GROUP_ID_CHUNK=<img src="https://latex.codecogs.com/gif.latex?{RUN_TS}_ber30min_chunked_prn1_spread_sr4p092e6_cf100e6_d1800sCAPTURE_DIR_CHUNK=/home/"/>USER/GNSS_RX_Data_local/<img src="https://latex.codecogs.com/gif.latex?TODAY_YEAR/"/>TODAY_DIR/<img src="https://latex.codecogs.com/gif.latex?CAPTURE_GROUP_ID_CHUNKLOCAL_STEM_CHUNK="/>CAPTURE_DIR_CHUNK/<img src="https://latex.codecogs.com/gif.latex?CAPTURE_GROUP_ID_CHUNKCAPTURE_NAME_SINGLE="/>{RUN_TS}_ber30min_single_prn1_spread_sr4p092e6_cf100e6_d1800s
CAPTURE_DIR_SINGLE=/home/<img src="https://latex.codecogs.com/gif.latex?USER/GNSS_RX_Data_local/"/>TODAY_YEAR/<img src="https://latex.codecogs.com/gif.latex?TODAY_DIR/"/>CAPTURE_NAME_SINGLE
LOCAL_STEM_SINGLE=<img src="https://latex.codecogs.com/gif.latex?CAPTURE_DIR_SINGLE/"/>CAPTURE_NAME_SINGLE
  
mkdir -p "<img src="https://latex.codecogs.com/gif.latex?CAPTURE_DIR_CHUNK&quot;%20&quot;"/>CAPTURE_DIR_SINGLE"
printf 'CAPTURE_DIR_CHUNK=<%s>\n' "<img src="https://latex.codecogs.com/gif.latex?CAPTURE_DIR_CHUNK&quot;printf%20&#39;CAPTURE_DIR_SINGLE=&lt;%s&gt;\n&#39;%20&quot;"/>CAPTURE_DIR_SINGLE"
```
