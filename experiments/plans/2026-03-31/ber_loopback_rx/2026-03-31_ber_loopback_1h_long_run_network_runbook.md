...
若按同一套 `sc16` 口径折算，中间几个常见时长大致为：

| 时长 | 总量级（十进制） | 总量级（二进制） | `30 s` chunk 数 |
|------|------------------|------------------|-----------------|
| `30 min` | `29.46 GB` | `27.44 GiB` | `60` |
| `45 min` | `44.19 GB` | `41.16 GiB` | `90` |
| `60 min` | `58.92 GB` | `54.88 GiB` | `120` |
...
如果把这个 `44G` 和中间时长对照起来看：

- `30 min` 总量约 `29.46 GB`，当前空间大体可承受
- `45 min` 总量约 `44.19 GB`，已经非常贴边，不适合作为正式目标
- `60 min` 总量约 `58.92 GB`，当前空间明确不足
