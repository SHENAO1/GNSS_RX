...
### 2.1 规格书参数摘录（b200-b210_spec_sheet.pdf）

| 参数 | 数值 | 单位 | 备注 |
|------|------|------|------|
| TX 输出功率（典型） | >10 | dBm | 单通道 |
| RX IIP3 | −20 | dBm | @典型噪声系数 |
| RX 1dB 压缩点（估算） | ≈ −30 | dBm | IIP3 − 10 dB 经验值 |
| SSB/LO 抑制 | −35/50 | dBc | — |
| RX 噪声系数 | <8 | dB | — |
| **RX 绝对最大输入功率** | **未注明** | — | **规格书缺失，保守估算不超过 +15 dBm** |

### 2.2 射频线直连功率预算

```
TX 输出（典型 tx_gain=20 dB）：约 +10 dBm
同轴线缆损耗：                 约 − 1 dB
───────────────────────────────────────────
RX 输入估算：                  约 +9 dBm
RX 1dB 压缩点（估算）：        约 −30 dBm
过驱动余量：                    +39 dB ← ⚠️ 严重饱和
```

### 2.3 强制安全要求

**结论：规格书中缺少 RX 绝对最大额定输入功率，直连时估计 RX 输入远超压缩点。**

**操作前必须在 TX 输出端串联衰减器：**

```
B210 TX 端口 → [30 dB 衰减器（首选）] → 同轴电缆 → B210 RX 端口
```

| 衰减方案 | 预期 RX 输入 | 安全判断 |
|----------|-------------|----------|
| 无衰减器 | ≈ +9 dBm | ❌ 严重超载，禁止 |
| 20 dB 衰减器 + tx_gain=20 | ≈ −11 dBm | ✅ 安全，留 19 dB 余量 |
| 30 dB 衰减器 + tx_gain=20 | ≈ −21 dBm | ✅ 安全，保守推荐 |
| 30 dB 衰减器 + tx_gain=40 | ≈ −1 dBm | ⚠️ 需谨慎，仍高于 1dB 压缩点 |

**若有频谱仪，先用频谱仪在衰减器输出端测量实际功率，再接 RX。**
...
**完成标志：**
- [ ] 已确认衰减器（≥20 dB）串联在 TX SMA 端口与同轴线缆之间
- [ ] 硬件连接顺序：TX → 衰减器 → 同轴线缆 → RX
- [ ] 若有频谱仪：已实测 TX 输出功率（填入记录）
- [ ] 两台 USRP 均被系统识别（`uhd_find_devices` 确认）
