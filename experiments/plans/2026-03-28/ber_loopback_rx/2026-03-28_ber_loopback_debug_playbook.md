...
## 2026-03-29 结论更新：主问题已收敛到采集连续性

当前新的联机 `30 s` 复验已经给出明确结论：

- 当 `TX 无 underflow` 且 `RX 无 overflow` 时，`tracked_truth` 已收敛到 `BER = 0.00e+00`（`0 / 1499 bit`）。
- 同一轮结果里 `tracked_match = 100%`，误码位置图为空，说明闭环链路本身已经打通。
- 因此，本手册旧小节中关于“参考比特序列不一致 / bit 边界假设错误”的排障路径，当前只在**无 overflow/underflow 但 BER 仍高**时才需要重新进入。
- 若本轮采集再次出现 overflow，应继续优先按“采集连续性被破坏”处理，而不是先回退到 TX pattern 或 truth 契约怀疑。
